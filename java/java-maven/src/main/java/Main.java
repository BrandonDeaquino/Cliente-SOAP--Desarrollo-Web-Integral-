...
public class Main {
        ...
        public void handle(HttpExchange exchange) throws java.io.IOException {
            try {
                ...
                try {
                    ...
                    String resultadoIngles = matcher.find() ? matcher.group(1).trim() : "No se encontró resultado";

                    String resultadoEspanol = traducirConGoogle(resultadoIngles);

                    exchange.sendResponseHeaders(200, resultadoEspanol.getBytes().length);
                    OutputStream os = exchange.getResponseBody();
                    os.write(resultadoEspanol.getBytes());
                    os.close();
    ...
    private static String traducirConGoogle(String texto) throws Exception {
        String textoCodificado = URLEncoder.encode(texto, StandardCharsets.UTF_8.toString());
        String url = "https://translate.googleapis.com/translate_a/single?client=gtx&sl=en&tl=es&dt=t&q=" + textoCodificado;
